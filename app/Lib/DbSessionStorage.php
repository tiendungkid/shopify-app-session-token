...
class DbSessionStorage implements SessionStorage
{
    ...
    {
        $dbSession = \App\Models\Auth\Session::query()->where('session_id', $sessionId)->first();
    ...
    {
        $dbSession = \App\Models\Auth\Session::query()->where('session_id', $session->getId())->first();
        if (!$dbSession) {
            $dbSession = new \App\Models\Auth\Session();
        }
        ...
        try {
            ...
            logger()->error('Failed to save session to database: ' . $err->getMessage());
            return false;
        }
    }
    ...
    {
        return \App\Models\Auth\Session::query()->where('session_id', $sessionId)->delete() === 1;
    }
}
